# Function to retrieve and process occurrence data from ALA for a given species
get_ala_occurrences <- function(species_name) {
  ...
  species_data <- occ(query = species_name, from = 'ala')
  ...
  # Check if we got data from ALA
  if(is.null(species_data) || 
     is.null(species_data$ala) || 
     is.null(species_data$ala$data) || 
     !(paste0(species_col_name) %in% names(species_data$ala$data)) ||
     nrow(species_data$ala$data[[paste0(species_col_name)]]) == 0) {
    
    message(paste("No occurrence data found for", species_name, "from ALA"))
    return(empty_df)
  }   
  ...
  result_df <- tryCatch({
    ...
    has_location <- location_col %in% names(species_data$ala$data)
    
    # Create dataframe with safe column access
    df <- as.data.frame(species_data$ala$data)
    
    processed_df <- df %>% 
      transmute(
        species = df[[paste0(species_col_name, ".name")]], 
        ...
        year = df[[paste0(species_col_name, ".year")]],
        coordinateUncertaintyInMeters = as.numeric(df[[paste0(species_col_name, ".coordinateUncertaintyInMeters")]]),
    ...
    processed_df$database_name <- "ala"
    processed_df$observation_count <- NA
    
    return(processed_df)
  }, error = function(e) {
    message(paste("Error processing ALA data for", species_name, ":", e$message))
    return(empty_df)
  })
...
get_all_caryota_occurrences <- function(species_list) {
  ...
  for (species in species_list) {
    ...
    species_df <- get_ala_occurrences(species)
...
# Get all occurrences
dfa_all <- get_all_caryota_occurrences(caryota_species)
